...
class Aggregate(nn.Module):  # MTN
    ...
    def forward(self, x):
        # x: (B, T, F)
        out = x.permute(0, 2, 1)  # train: out.shape=[640,1024,32]
        residual = out
        ...
        out = torch.cat((out_d, out), dim=1)  # train: out.shape=[640,1024,32]
        out = self.conv_5(out)  # fuse all the features together, train: out.shape=[640,1024,32]
        out = out + residual
        out = out.permute(0, 2, 1)  # train: out.shape=[640,32,1024]
        # out: (B, T, 1)
        return out
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.Aggregate_audio = Aggregate(len_feature=128)
        if self.feature_group == 'both':
            if args.fusion == 'concat':
                self.fc1 = nn.Linear(args.feature_size + args.emb_dim + 128, 512)
    ...
    def forward(self, inputs, text, audio):
        ...
        k_abn = self.k_abn
        k_nor = self.k_nor
        out = inputs  # shape=[64,5,32,1024]
        bs, ncrops, t, f = out.size()  # t is no. of clips
        bs2, ncrops2, t2, f2 = text.size()
        bs3, ncrops3, t3, f3 = audio.size()

        out = out.view(-1, t, f)
        out2 = text.view(-1, t2, f2)
        out3 = audio.view(-1, t3, f3)

        out = self.Aggregate(out)  # train: out.shape=[640,32,1024]
        out = self.drop_out(out)  
        
        if self.aggregate_text:
            out2 = self.Aggregate_text(out2)  # train: out2.shape=[640,32,768]
            out2 = self.drop_out(out2)  

        if self.aggregate_audio:
            out3 = self.Aggregate_audio(out3)   # train: out3.shape=[640,32,128]
            out3 = self.drop_out(out3)

       
        if out.shape[1] < out2.shape[1]:  # out(vis) out2(text)
            # remove the last frame of out2
            out2 = out2[:, :(out.shape[1] - out2.shape[1]), :]
        elif out.shape[1] > out2.shape[1]:  # out(vis) out2(text)
            # padding out2 by repeating the last frame
            out2 = torch.cat((out2, out2[:, (out2.shape[1] - out.shape[1]):, :]), dim=1)

        if out.shape[1] < out3.shape[1]:
            out3 = out3[:, :(out.shape[1] - out3.shape[1]), :]
        elif out.shape[1] > out3.shape[1]:
            out3 = torch.cat((out3, out3[:, (out3.shape[1] - out.shape[1]):, :]), dim=1)
        t = out.shape[1]


        if self.fusion == 'concat':
            if self.feature_group == 'both':
                out = torch.cat([out, out2, out3], dim=2)  # train: out.shape=[64, 5, 32, 1024+768+128]
            elif self.feature_group == 'text':
        ...
        scores = scores.view(bs, ncrops, -1).mean(1)  # train: scores.shape=[64,5,32] ->[64,32]
        scores = scores.unsqueeze(dim=2)  # train: scores.shape=[64,32,1]

        normal_features = features[0:self.batch_size * ncrops]  # train: normal_features.shape=[320,32,1024]
        normal_scores = scores[0:self.batch_size]  # train: normal_scores.shape=[32,32,1]

        abnormal_features = features[self.batch_size * ncrops:]  # train: abnormal_features.shape=[320,32,1024]
        ...
        idx_abn_feat = idx_abn.unsqueeze(2).expand([-1, -1, abnormal_features.shape[2]])  # train: shape=[32,3,1024]

        abnormal_features = abnormal_features.view(n_size, ncrops, t, -1)  # train: shape=[32,5,32,1024]
        abnormal_features = abnormal_features.permute(1, 0, 2, 3)  # train: shape=[5,32,32,1024]

        total_select_abn_feature = torch.zeros(0)
        for abnormal_feature in abnormal_features:  # range(5)
        ...
        score_abnormal = torch.mean(torch.gather(abnormal_scores, 1, idx_abn_score),
                                    dim=1)  # train: shape=[32,3,1] mean[32,1], top 3 scores in abnormal bag based on the top-3 magnitude
        ...
        feat_select_abn = total_select_abn_feature  # train: shape=[320,3,1024]
        feat_select_normal = total_select_nor_feature  # train: shape=[320,3,1024]
