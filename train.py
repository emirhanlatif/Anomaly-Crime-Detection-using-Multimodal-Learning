...
def train(nloader, aloader, model, args, optimizer, viz, device):
    with torch.set_grad_enabled(True):
        ...
        ninput, ntext, naudio , nlabel = next(nloader)  # ninput.shape=[32,10,32,2048], nlabel.shape=[32,]
        ainput, atext, aaudio, alabel = next(aloader)  # ainput.shape=[32,10,32,2048], alabel.shape=[32,]
        ...
        feat_normal_bottom, scores, scores_nor_bottom, scores_nor_abn_bag, _ = model(input, text, audio)  # b*32 x 2048
